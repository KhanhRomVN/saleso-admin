...
const CategoriesPage: React.FC = () => {
  ...
  const fetchCategories = async (level = 1) => {
    try {
  ...
  const fetchChildCategoriesForSelect = async (parentId: string) => {
    try {
      const response = await axios.get(
        `http://localhost:8080/category/children-of-parent/${parentId}`
      );
  ...
  const handleCreateCategory = async (category: Category) => {
    try {
      const dataCategory = {
        ...
        image_uri: newCategory.image_uri,
        description: newCategory.description,
        parent_id: category._id,
        level: category.level + 1,
      };
      // await axios.post("http://localhost:8080/category/create", dataCategory);
      setNewCategory({});
      fetchCategories();
    } catch (error) {
  ...
  const handleInsertCategory = async (category: Category) => {
    try {
      const dataCategory = {
        ...
        parent_id: category._id,
        level: category.level + 1,
        children_id: newCategory.children_id,
      };
      console.log(dataCategory);

      // await axios.post("http://localhost:8080/category/insert", dataCategory);
      // setNewCategory({});
      // fetchCategories();
    } catch (error) {
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="grid grid-cols-4 gap-4">
        {categories.map((category) => (
          <div
            key={category._id}
            className="p-4 border rounded cursor-pointer group relative bg-blue-400"
          >
            <h2
                ...
                <DialogContent>
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea
                      ...
                      placeholder="Description"
                    />
                    <Button onClick={() => handleCreateCategory(category)}>
                      Create
                    </Button>
                  </div>
                </DialogContent>
                ...
                <DialogContent>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onOpenChange={(open) => {
                        if (open) {
                          fetchChildCategoriesForSelect(category._id);
                        }
                      }}
                    >
                      <SelectTrigger>
                    ...
                    <Button onClick={() => handleInsertCategory(category)}>
                      Insert
                    </Button>
                  </div>
                </DialogContent>
